...
class PySpinCam(object):
    def __init__(self, cam, manager, idx):
        if cam is None:
            raise ValueError('Camera is None or already in use.')
        self.cam = cam
    ...
    #     # Stop acquisition only if the camera is currently acquiring images
    ...
    def release(self):
        ...
        self.cam.DeInit()
        self.cam = None
        self.manager.release_camera(self.idx)
